...
static uint64_t get_pdflags(uint32_t memtype)
{
  ...
  case PMEMTYPE_UNCACHED:
    return PF_PRESENT | PF_GLOBAL | PF_SYSTEM | PF_RW | PF_PS | PF_PWT | PF_PCD;    
  case PMEMTYPE_BAD:
...
static uint64_t get_ptflags(uint32_t memtype)
{
  ...
  case PMEMTYPE_ACPI_NVS:
  case PMEMTYPE_UNCACHED:
    return PF_PRESENT | PF_GLOBAL | PF_SYSTEM | PF_RW | PF_PWT | PF_PCD;
  case PMEMTYPE_BAD:
  case PMEMTYPE_UNMAPPED:
    return 0;    
  case PMEMTYPE_USABLE:
...
/// its virtual address.
static inline uint64_t alloc_page(pagetable_t *pt)
{
...
static void map_region(pagetable_t *pt, const pmap_t *map, const pmapregion_t *region)
{
  ...
  if (region->type == PMEMTYPE_RESERVED && region->addr >= map->last_usable) {
    return;
  }
