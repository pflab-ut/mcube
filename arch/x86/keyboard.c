...
struct kbstate {
  ...
  keyboard_t        buf[MAX_BUFSIZ]; ///< Buffer holding unconsumed keys.
};
...
static inline void toggle(uint8_t flag)
{
  if (state.meta & flag) {
    state.meta &= ~flag;
  } else {
...
static void isr_keyboard(const interrupt_context_t *context)
{
  ...
  uint8_t scancode = inb(KB_PORT_DATA);
