...
[map all ./build/loader.map]

; Include constants, structures, and macros.
%include "include/x86/mem.inc"          ; Memory layout constants
%include "include/x86/globals.inc"      ; Global variable definitions
%include "include/x86/bios.inc"         ; BIOS structures
%include "include/x86/iso9660.inc"      ; ISO9660 structures
%include "include/x86/gdt.inc"          ; Global descriptor table structures
...
; FindKernel
;
; Scan the root directory of the cdrom for a file called "MCUBE.SYS". If found,
...
FindKernel:
    ...
    .processDirEntry:
        ...
        ; Is the file name the same length as "MCUBE.SYS;1"?
        cmp     byte [di + ISO.DirectoryEntry.NameLength], \
                    Kernel.Filename.Length
        jne     .nextDirEntry

        ; Is the file name "MCUBE.SYS;1"?
        push    di
...
Kernel.Filename         db      "MCUBE.SYS;1"
