...
void pubsub_main(void)
{
  ...
  if (own.cluster_id == 0) {
    ...
    for (int i = 1; i < NR_CLUSTERS; i++) {
      ...
      /*dmac_write_to_cluster((uint32_t) &is_pubsub_ready, (uint32_t) high_addr,
                            (uint32_t) low_addr, sizeof(is_pubsub_ready), 0, 0, DMAC_POLLING);*/
    }
    ...
    while (true) {
      if (flag != psnode.buf[3]) {
        ...
        for (int i = 2; i < NR_CLUSTERS; i++) {
          ...
          dmac_write_to_cluster((uint32_t) psnode.buf, (uint32_t) high_addr,
                                (uint32_t) low_addr, sizeof(char)*4, 0, 0, DMAC_POLLING);
        }
